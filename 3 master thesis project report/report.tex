...
applications to produce efficient native programs that are significantly faster
than the default JVM-based runtime offered by Scala.
...
removed verification conditions, such as \InlineS{require} or
\InlineS{invariant}, in order to have a fair comparison of runtime performances
between the JVM and C backends. These changes resulted in runtimes are around
12s and 10s on the JVM, respectively for the \textbf{Designs A \& B}, of user
time as reported by the \Inline{time} utility.
...
\RefFig{fig:lzw-res} reports these results.
...
code and generated two native versions using both Clang and GCC compilers, at
the \Inline{-O3} optimisation levels. In order to allow the JVM to perform
...
\emph{jvm\_no\_vc}, as a baseline. The data series named \emph{jvm\_vc} indicate
the runtime cost of verification conditions that Leon could prove to be correct
...
the performance of \emph{Scala Native} \cite{scala_native}.
...
parameters. In the third column, we plotted the speedups across inputs, where
the smallest input image is on the left of the x-axis and the largest on the
right. For each row of the figure, we present the result for a different
...
Leon, this is not an issue here: we can statically check that the verification
conditions present in the source code are valid or not. Hence, we can remove
such assertions from the executed program once we are satisfied with the quality
...
performance as the JVM runtime denoted by \emph{jvm\_no\_vc}: all programs lasts
...
Further, we also removed the verification conditions so that the runtime
performances reported by \emph{native} in \RefFig{fig:improc-res} are comparable
to the \emph{jvm\_no\_vc}, \emph{clang.3} and \emph{gcc.3} series.

However, and this can be explained by the experimental nature of version 0.1 of
...
by a factor 2x. It is expected that the runtime performance of \emph{Scala
...
ability to target hardware not supported by the JVM.
...
algorithm and an algorithm to apply filters on images, illustrating how
...
optimisation strategies, result in runtime up to 2.5x faster compared to the
usual JVM runtime offered by Scala, when runtime assertions are removed, while
being as safe as the source programs. Without Leon's ability to prove the
correctness of our benchmark at compile time, the runtime assertions hinder
performance by a factor reaching 10x when compared to the runtime of the
generated C99 code that is exempts of such costly checks.

Hence, we showed that one can write safe, efficient programs written in Scala
using Leon for verification purposes and its \GenC module to generate equivalent
C99 programs.
