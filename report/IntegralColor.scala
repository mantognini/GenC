...
object IntegralColor {
  ...
  def testColorWholeImage: Boolean = {
    ...
    // NOTE: Cannot define a toGray function as XLang
    // doesn't allow mutating arguments and GenC doesn't
    ...
    (while (idx < WIDTH * HEIGHT) {
      ...
      idx >= 0 && idx <= WIDTH * HEIGHT &&
      gray.length == WIDTH * HEIGHT
    }
    // NB: the last invariant is very important;
    // without it the verification times out
  ...
  def applyFilter(gray: Array[Int], size: Int,
                  idx: Int, kernel: Kernel): Int = {
  ...
  def testFilterConvolutionSmooth: Boolean = {
    ...
    // NOTE: Cannot define a `smoothed` function as XLang
    // doesn't allow mutating arguments and GenC doesn't
    // allow returning arrays
    ...
    (while (idx < smoothed.length) {
      ...
      idx >= 0 && idx <= smoothed.length &&
      smoothed.length == gray.length
    }
  ...
  def main: Int = {
    if (testColorSinglePixel &&
        testColorWholeImage &&
        testFilterConvolutionSmooth) 0
    else 1
  } ensuring { _ == 0 }
